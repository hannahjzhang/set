public class Grid {
  ...
  public void removeSet() {
    ...
    Location a = selectedLocs.get(0);
    Location b = selectedLocs.get(1);
    Location c = selectedLocs.get(2);
    int finalCols = cardsInPlay / ROWS;
    if ((cardsInPlay > 12) || (deck.size() == 0)) {
      // find out how many are in the last column
      int counter = 0;
      //if (a.getCol() == finalCols-1) {
      //  counter++;
      //}
      //if (b.getCol() == finalCols-1) {
      //  counter++;
      //}
      //if (c.getCol() == finalCols-1) {
      //  counter++;
      //}
      for (int i = 0; i < selectedLocs.size(); i++) {
        if (selectedLocs.get(i).getCol() == finalCols - 1) {
          counter++;
        }
      }
      // based on how many selectedLoc are in the last column, consolidate
      if (counter == 3) {
        cardsInPlay = cardsInPlay - 3;
        currentCols--;
      }
      // find the one that is not selected and move
      else if (counter == 2) {
        int rowToMove;
        if ((a.getRow() != 0) && (b.getRow() != 0) && (c.getRow() != 0)) {
          rowToMove = 0;
        }
        else if ((a.getRow() != 1) && (b.getRow() != 1) && (c.getRow() != 1)) {
          rowToMove = 1;
        }
        else {
          rowToMove = 2;
        }
        board[a.getCol()][a.getRow()] = board[finalCols-1][rowToMove];
        cardsInPlay = cardsInPlay - 3;
        currentCols--;
      }
      // one selectedLocation in the last column, must be last in list
      else if (counter == 1) {
        int rowToStay = c.getRow();
        if (rowToStay == 0) {
          board[a.getCol()][a.getRow()] = board[finalCols-1][1];
          board[b.getCol()][b.getRow()] = board[finalCols-1][2];
        }
        else if (rowToStay == 1) {
          board[a.getCol()][a.getRow()] = board[finalCols-1][0];
          board[b.getCol()][b.getRow()] = board[finalCols-1][2];
        }
        else {
          board[a.getCol()][a.getRow()] = board[finalCols-1][1];
      ...
      // counter is zero
        ...
        board[a.getCol()][a.getRow()] = board[finalCols-1][0];
        board[b.getCol()][b.getRow()] = board[finalCols-1][1];
        board[c.getCol()][c.getRow()] = board[finalCols-1][2];
        cardsInPlay = cardsInPlay - 3;
        currentCols--;
      }
    }
    else {
      board[a.getCol()][a.getRow()] = deck.deal();
      board[b.getCol()][b.getRow()] = deck.deal();
      board[c.getCol()][c.getRow()] = deck.deal();
    }
  }
  ...
  public void addCardToBoard(Card card) {
    // since cardsInPlay starts at 0 so no need to subtract
    int thisCol = (cardsInPlay) / ROWS;
    int thisRow = (cardsInPlay) % ROWS;
    board[thisCol][thisRow] = card;
    cardsInPlay++;
